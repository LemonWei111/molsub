...
# could not install apex0.1 with python3.6
...
def test(args, model, test_loader):
    ...
    my_label = all_label[:, 0]
    my_all_property = all_property[:, 0]
    # my_all_preds = all_preds[:,0]
    label_path = "./label_prob/"+ args.name+ "/" + args.pretrained_dir.split('/')[-1].split('.')[0] +".txt"
    with open(label_path,"w",encoding="utf-8") as fw:
        for i in range(len(my_label)):
            fw.write(str(my_label[i]) + '\t' + str(my_all_property[i]) + '\n')
    
    result_path =  "./result/"+ args.name+ "/" + args.pretrained_dir.split('/')[-1].split('.')[0] +".txt"
    with open(result_path,"w",encoding="utf-8") as fw:
        fw.write("Valid Loss:" + '\t' + str(eval_losses.avg) + '\n')
        fw.write("The average AUROC:" + '\t' + str(auroc_avg) + '\n')
        for i in range(args.num_classes):
            fw.write("The AUROC of " + CLASS_NAMES[i] + ' is ' + str(aurocs[i]) + '\n')
...
def main():
    ...
    parser.add_argument("--num_classes", default=2, type=int, help="the number of class")                    
    ...
    if args.local_rank == -1:
        ...
        args.n_gpu = 0 # torch.cuda.device_count()
